...
class MenuController extends Controller
{
    ...
    public function create_menu(Request $request)
    {
    	...
    	$result=DB::connection('myshop')->table('menu')->insert([
    		'name1'=>$req['name1'],
    		'name2'=>$req['name2'],
    		'type'=>$req['type'],
  ...
  public function load_menu()
    {
        $data = [];
        $menu_list = DB::connection('myshop')->table('menu')->select(['name1'])->groupBy('name1')->get();
        foreach($menu_list as $vv){
            $menu_info = DB::connection('myshop')->table('menu')->where(['name1'=>$vv->name1])->get();
            $menu = [];
